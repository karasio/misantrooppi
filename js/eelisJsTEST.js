...
function placeMarkers() {
  ...
  let markerStart = new L.marker(
      [coordinates.from.lat, coordinates.from.lon]).addTo(map);
  let markerEnd = new L.marker([coordinates.to.lat, coordinates.to.lon]).addTo(
      map);
  ...
  for (let j = 0; j < stopsOnRoute.length; j++) {
    ...
    for (let k = 1; k < stopsOnOneRoute.length - 1; k++) {
      ...
      if (stopsOnOneRoute[k].name === stopsOnOneRoute[x].name) {
        ...
        stopsOnRouteList += '<li class="' + vehicleClass + '">' +
            stopsOnOneRoute[k].name + stopsOnOneRoute[k].vehicle + '</li>';
      } else {
        stopsOnRouteList += '<li class ="' + vehicleClass + '">' +
            stopsOnOneRoute[k].name + stopsOnOneRoute[k].vehicle + '</li>';
      }
